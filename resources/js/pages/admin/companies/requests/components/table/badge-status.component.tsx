...
const badgeVariants = cva('text-[0.65rem] capitalize leading-none line-height-none py-1 pl-5 pr-1 relative', {
    variants: {
        status: {
...
type BadgeStatusProps = ComponentProps<typeof Badge> &
...
export default function BadgeStatus({ className, status, ...props }: BadgeStatusProps) {
    ...
    return (
        <Badge
            variant="outline"
            data-status={status}
            className={cn(className, badgeVariants({ status }))}
            title={RegistrationStatusLabel(status)}
            {...props}
        >
            {renderIcon()}
