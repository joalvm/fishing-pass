...
export default function TableFooter() {
    ...
    const remainingRows = per_page - requests.data.length;
                ...
                <TableCell colSpan={7}>
                    <div className="flex items-center justify-end space-x-6 lg:space-x-8">
                        ...
                        <div className="flex items-center space-x-2">
                            <Select value={`${per_page}`} onValueChange={(value) => setPerPage(Number(value))}>
                        ...
                        <div className="flex items-center space-x-2">
                            <Button variant="outline" className="hidden h-8 w-8 p-0 lg:flex" onClick={() => setPage(1)} disabled={!canGoPrevious}>
                                <span className="sr-only">Ir a la primera página</span>
                                <ChevronsLeft className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" className="h-8 w-8 p-0" onClick={() => setPage(current_page - 1)} disabled={!canGoPrevious}>
                                <span className="sr-only">Ir a la página anterior</span>
                                <ChevronLeft className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" className="h-8 w-8 p-0" onClick={() => setPage(current_page + 1)} disabled={!canGoNext}>
                                <span className="sr-only">Ir a la página siguiente</span>
                                <ChevronRight className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" className="hidden h-8 w-8 p-0 lg:flex" onClick={() => setPage(last_page)} disabled={!canGoNext}>
